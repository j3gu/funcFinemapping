...
for(trait in test){
  ...
  snpsIn<-read.table(sprintf("output/AAD/%s/Caldero2019_lineage_snps.sumstats", trait), header=T)
  snpsIn["term"]<-unlist(lapply(rownames(snpsIn), function(i){strsplit(i, "_d")[[1]][1]}))
  colnames(df)<-c("raw_term", "estimate", "low", "high")
  ### remove lines of FDR values for each loci
  df["label"] = unlist(lapply(df$raw_term, function(i){length(strsplit(i[1], "[.]")[[1]])>1}))
  df<-df[df$label==TRUE,]
  ...
  df<-inner_join(df, snpsIn, by="term")
  tbl<-data.frame(rbind(tbl, cbind(df, trait)))
}
...
```{r echo=F}
abs_estimate<-exp(tbl$estimate)
tbl["h2g"]<-round(abs_estimate * tbl$percent*100, 1)
```
...
```{r echo=F}
# celltypes<-c("B_cell_compare", "T_cell_compare", "NK_compare", "Myeloid_compare")
# summary<-c()
# for (trait in traits){
#   output<-c()
#   for(cell in celltypes){
#     f<-read.table(sprintf("~/cluster/projects/torus_enrichment/%s/Caldero2019/%s/snps_summary.txt", trait, cell), header=T)
#     terms<-unlist(lapply(rownames(t(f[2,])), function(i){strsplit(i, "_d")[[1]][1]}))
#     output<-rbind(output,t(f[2,]))
#   }
#   terms<-unlist(lapply(rownames(output), function(i){strsplit(i, "_d")[[1]][1]}))
#   new_output<-data.frame(output, row.names = NULL)
#   colnames(new_output)<-"percent"
#   new_output["term"]<-terms
#   new_output["trait"]<-trait
#   summary<-rbind(summary,new_output)
# }
 ```
...
ggplot(tbl[tbl$trait %in% test,], aes(x=celltype, y=h2g, colour=annot)) + geom_point() + facet_grid(. ~ trait) + ylab("Heritability(%)")
ggplot(tbl[tbl$trait %in% control,], aes(x=celltype, y=h2g, colour=annot)) + geom_point() + facet_grid(. ~ trait) + ylab("Heritability(%)")
```
