...
## setting working directory (not functioning)
knitr::opts_knit$set(root.dir = "/home/jinggu/projects/funcFinemapping")
```
...
traits<-c("allergy", "asthma_adult", "asthma_child", "bmi", "height", "LDL", "HDL")
test<-traits[1:3]
control<-traits[5:8]
tbl<-data.frame()
for(trait in test[1:2]){
...
for(trait in traits){
  ...
  df["term"]<-unlist(lapply(df$tmp, function(i){
    return(sprintf("%s(%s)", i, snpsIn[snpsIn$term == i, "percent"]))
    }))
  df["percent"]<-unlist(lapply(df$tmp, function(i){
    return(snpsIn[snpsIn$term == i, "percent_val"])
  }))
  df["celltype"]<-unlist(lapply(df$tmp, function(i){strsplit(i, "[_]")[[1]][1]}))
  df["annot"]<-unlist(lapply(df$tmp, function(i){rev(strsplit(i, "[_]")[[1]])[1]}))
  df["h2g"]<-round(exp(df$estimate)*df$percent*100, 1)
  tbl<-data.frame(rbind(tbl, cbind(df, trait)))
}
...
```
**Percent of genetic signal**
```{r echo=F,fig.width=10}
ggplot(tbl[tbl$trait %in% c(test[2:3], "bmi","height") & !(tbl$celltype %in% c("GMP-A", "GMP-B", "GMP-C", "pDC")),], aes(x=celltype, y=h2g, fill= c(annot))) + 
    geom_bar(stat="identity") + ylab("Percent of genetic signal") + 
...
ggplot(tbl[tbl$trait %in% c("bmi","height") & !(tbl$celltype %in% c("GMP-A", "GMP-B", "GMP-C", "pDC")),], aes(x=celltype, y=h2g, fill= c(annot))) + 
    geom_bar(stat="identity") + ylab("Percent of genetic signal") + 
    scale_fill_discrete(name="Annotation sets", labels=c("Blood Only", "Common", "Lung Only"), type= palette.colors(palette="Set3")[c(1,6,5)]) + 
    theme(axis.text.x=element_text(angle=45, vjust = 0.9, hjust=0.8)) + 
    facet_grid(.~trait) + theme(strip.text.x = element_text(size=14))
...
```
**troubleshoot for >100% genetic signal in GMP-merge annotation**
```{r echo=f}
df<-read.table("output/AAD/GMP_merge_stats.txt", header=T)
df
tbl<-read.table("/home/jinggu/cluster/projects/torus_enrichment/asthma_adult/Ulirsch2019/GMP_merge_compare/torus_joint_enrichment.est", header=F, skip=5)

```

## stratified-LDSC for partitioning h2g
### Joint run of all annotations in Wang2020
In LDSC tutorial for cell type specific analysis, it is recommended to include a “control” set of genes or chromatin accessibility peaks in our case for estimating heritability. The point is to use the specifically accessible peaks to each cell type. Just to let you know I have not done that in this run of analysis.
```{r echo=F}
...
for(trait in test[1:3]){
  ...
  new.f<-cbind(trait, f[f$Category %in% rev(f$Category)[1:length(labels$V1)],])
  new.f["term"]<-unlist(lapply(new.f$Category, function(i){
    return(labels$V1[as.numeric(strsplit(i, "_")[[1]][2])])
  }))
  tbl<-data.frame(rbind(tbl, new.f))
}
orders<-rev(c("B_cell","T_cell","natural_killer_cell",
                                  "macrophage", "erythrocyte",
                                  "club_cell","basal_cell","ciliated_cell", 
                                  "neuroendocrine_cell","type_II_pneumocyte","type_I_pneumocyte",
                                  "capillary_endothelial_cell", "endothelial_cell_of_artery",
                                  "endothelial_cell_of_lymphatic_vessel",
                                  "extracellular_matrix_secreting_cell","myofibroblast_cell","pericyte_cell"
                                  ))
colnames(tbl)[c(6,7)]<-c("estimate", "sd")
tbl["high"]<-tbl$estimate + tbl$sd
tbl["low"]<-tbl$estimate - tbl$sd

snp_enrichment_plot(tbl,
                    log.based = F,
                    text.size = 10,
                    y.order = orders,
                    trait = "")  +
...
```
This figure is based on the joint analysis using S-LDSC. The x-axis is in absolute scale. The y-axis contains proportion of SNPs that fall into the open chromatin peaks in each cell type. Major immune cell types show significant enrichment except for erythrocytes, of which the enrichment patterns greatly differ between child and adult-onset asthma. Macrophage cells explain twice amount of heritability in child-onset asthma than the adult-onset asthma. Overal, each immune cell types contribute about 8-10% of heritability to adult-onset asthma. 
### Ulirsch2019
**Enrichment estimates**
```{r echo=F}
feature<-c("B", "CD4","CD8","Ery","GMP_merge","mDC", "Mega","Mono")
h2g<-c()
for(i in feature){
  for(trait in test[1:3]){
    tbl<-read.table(sprintf("output/AAD/%s/Ulirsch2019/%s_%s.results", 
                            trait, trait, i), header=T)
...
```{r echo=F}
feature<-c("B", "CD4","CD8","Ery", "GMP-merge", "mDC", "Mega","Mono")
ylabel<-c("B", "CD4+T","CD8+T","Ery","GMP", "mDC","Mega","Mono")
...
**h2 explained**
```{r echo=F, fig.width=10}
h2g$celltype<-factor(h2g$celltype, levels=ylabel)
