...
summarize_results<-function(trait){
  tbl<-c()
  for(i in cutoffs[1:(length(cutoffs)-1)]){
    f<-read.table(sprintf("%s/%s/torus_enrichment.%s.est", outDIR, trait, as.character(i)))
...
snp_enrichment_plot(summarize_results("scz_2018"), trait="SCZ") + ggtitle("Torus Enrichment Results")
```
SpliceAI predictions with a cutoff lower than 0.05 show enrichment with SCZ risk variants. This implies SCZ risk variants are less likely to affect traits by disrupting mRNA splicing.

```{r echo=F}
plot_enrichment<-function(output.tbl, trait){
 p <- ggplot(output.tbl, aes(y=term, x=log2(exp(estimate)))) +
