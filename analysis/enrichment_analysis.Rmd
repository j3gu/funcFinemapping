...
```{r echo=F}
outDIR="/home/jinggu/cluster/projects/torus_enrichment/aFib/mmsplice"
df<-read.table(sprintf("%s/torus_enrichment_agg.est", outDIR), header=F)
colnames(df)<-c("raw_term", "estimate", "low", "high")
df["label"]<-paste("mmsplice", seq(0.4, 1.8, 0.2), sep="_")
df["numOfVariants"]<-c(2103, 1124, 728, 504, 371, 315, 266, 226)
df["term"]<-sprintf("%s(%s)", df$label, df$numOfVariants)
snp_enrichment_plot(df[1:4,], trait="AFib")
```
**MTSplice - tissue-specific **
For AFib, I took an average of MTSplice predicted scores for Atrial Appendage - Heart and Left Ventricle - Heart. 
```{r echo=F}
outDIR="/home/jinggu/cluster/projects/torus_enrichment/aFib/mtsplice"
df<-read.table(sprintf("%s/torus_enrichment_agg.est", outDIR), header=F)
colnames(df)<-c("raw_term", "estimate", "low", "high")
df["label"]<-paste("mtsplice", seq(1.0, 2.0, 0.2), sep="_")
df["numOfVariants"]<-c(22389, 17157, 14155, 12019, 10414, 9081)
df["term"]<-sprintf("%s(%s)", df$label, df$numOfVariants)
