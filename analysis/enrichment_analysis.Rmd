...
Background: 
From literature, only a limited fraction between 7 to 21% of splicing QTLs are tissue-specific (Meme et al.2015). The quantity for measuring variant effect on splicing is the inclusion level of the exon, denoted by percent spliced in (PSI $\Psi$). It was defined as the fratcion of the number of overlapped reads (IR) out of total reads, which are the sum of overlapped reads (IR) and split reads (ER) that are aligned to multiple exons due to alternative splicing.
$$
\Psi = \frac{IR}{IR+ES}
$$
When tissue-specific splicing was predicted using TSplice, the author found out less than 3.4% tested pairs of tissue-specific splicing scores deviated by 20% from the tissue-averaged splicing scores. In terms of the performance, TSplice's predictions is positively correlated with measurements (mean $\rho=0.22$). By combining the models of tissue-specific and tissue-indenpendent effects on splicing (called as MTSplice), they were able to achieve significant improvement, especially in 10 out of 12 brain tissues. However, the improvement in relevant mean square error was modest.
...
##### Testrun on SCZ GWAS    
**MMSplice - not tissue-specific **
This test run was based on splicing annotations generated using a tissue-independent model. For SCZ, we observed the enrichment estimates take positive values and their standard deviations were smaller compared to AFib's risk variants. 
```{r echo=F}
tbl<-read.table("output/splicing/scz_numVar_in_mmsplice_pred.txt", header=F)
colnames(tbl)<-c("cutoff(absolute value)", 
                 "num_gwasVar_in_annot", 
                 "num_randomVar_in_annot")
N = 8318559
...
colnames(df)<-c("raw_term", "estimate", "low", "high")
df["label"]<-paste("mmsplice", c(seq(0.2, 1.8, 0.4), 2.0), sep="_")
df["counts"]<-tbl$num_gwasVar_in_annot
df["term"]<-sprintf("%s(%s)", df$label, df$counts)
snp_enrichment_plot(df)
```
**Number of Variants in tisse-specific splicing annotations**
We counted the number of variants with the predicted splicing scores greater than each cutoff across multiple brain tissues. We observed greater variations across tissues at a lower cutoff. 
```{r echo=F, message=F}
tbl<-read.table("output/splicing/numVar_in_annot_by_tissue.txt", header=T)
tbl["cutoff"]<-c(seq(0.2, 2, 0.4), 2.0)
df<-melt(tbl[-1,], "cutoff")
ggplot(df, aes(x=variable, y=value, fill=factor(cutoff))) + xlab("") + 
  geom_bar(stat="identity", position=position_dodge()) + 
  theme(axis.text.x=element_text(angle=90, vjust = +0.7)) + 
  ylab("Number of variants in Annotations") 
```
**Enrichment of SCZ risk variants in brain-specific annotations**
The brain-specific annotations were binary that it was set to be 1 if $|\Delta\text{logit}(\Psi)|$ >=0.6 or zero otherwise. Though the tissue-indenpendent predictions of variants effects on splicing show no significant enrichment with risk variants, we see different levels of enrichments on the combined predictions across multiple brain tissues as well as whole blood. 
```{r echo=F}
...
snp_enrichment_plot(df)
```
**Joint run over tissue-independent and combined predictions**
```{r echo=F, fig.width=10}
df<-read.table("output/splicing/torus_enrichment_joint_Hypotha.est", header=F)
colnames(df)<-c("term", "estimate", "low", "high")
snp_enrichment_plot(df[-1,], y.label = c("MMsplice(tissue average)", "MTSplice(Combined)")) + 
  ggtitle("Enrichment of splicing predictions specific to Hypothalamus-Brain tissue")
```
