...
2. Prediction of 3D genome folding from DNA sequences:    
    * deep learning approach - train Hi-C/Micro C and 1D DNA sequences and then predict on any DNA sequence
    * an example shown for rs339331
[!comparison in contact maps surrounding rs339331 between ref and alt alleles](assets/contact_maps_rs33933.png)
...
```
Enrichment of m6A regions from different tissues in GWAS of SCZ

* marginal enrichment estimates
```{r echo=F}
df<-read.table("data/torus_enrichment_m6A.est", header = F)
colnames(df)<-c("term", "estimate", "low", "high")
library(ggplot2)
ggplot(df, aes(y=reorder(term, estimate), x=log2(exp(estimate)))) +
       geom_errorbar(aes(xmax=log2(exp(high)), xmin=log2(exp(low))),width=0.2)+
       geom_point(size=1.5) + xlab("SCZ variant enrichment(log2)") + 
       theme_bw() + ylab("") + 
       scale_y_discrete(labels=c("m6A_Brain", "m6A_Muscle", "m6A_Kidney", "m6A_Placenta", "m6A_Heart", "m6A_Stomach", "m6A_Lung", "m6A_Liver"))  
...
for(i in c(neg_controls, test)){
  outputList[[i]]<-read.table(paste(outDIR, sprintf("%s/joint_0610/torus_joint_enrichment.est", i),sep="/"), header = T)
...
plot_enrichment(c("scz_2018", "ibd_IBD", "ibd_UC", "ibd_CD"), log_based = T) + xlim(c(-6,6))
...
tbl<-df[df$Category %in% c("CDS_0", "PromoterFlanking_Hoffman.flanking.500_0", "CADD_0", "iPSC_0", "human_CG_DMRs_0"),]
ggplot(tbl, aes(y=reorder(Category, Enrichment), x=Enrichment)) + 
       ...
       scale_y_discrete(labels=c("Promoter", "iN-OCR", "CDS", "CADD", "Brain-DMR")) +
