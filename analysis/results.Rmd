...
# f<-fread("~/resources/genomes/CDTS_1per_genome_overlaps_full.bed")
# out1<-unlist(lapply(split(f, f$V5), function(i){length(unique(i$V1))/counts_1per}))
# 
# f<-fread("~/resources/genomes/CDTS_5per_genome_overlaps_full.bed")
...
### Joint enrichment analyses on lipid or immune related traits 
...
neg_controls<-c("bmi", "height", "ibd_IBD", "ibd_UC", "HDL", "LDL")
test<-"scz_2018"
plot_enrichment_est<-function(trait){
  df<-read.table(paste(outDIR, sprintf("%s/joint_CDTS_noncoding/torus_joint_enrichment.est", trait),sep="/"), header = T)
  p<-ggplot(df[-1,], aes(y=reorder(term, estimate), x=log2(exp(estimate)))) +
       geom_errorbar(aes(xmax=log2(exp(high)), xmin=log2(exp(low))),width=0.2)+
       geom_point(size=1.5) + xlab("SCZ variant enrichment(log2)") + 
       theme_bw() + ylab("")  + #xlim(c(xmin, xmax))+
       geom_vline(xintercept=0, linetype="dashed", color="grey")
  return(p)
}

for(i in c("bmi", "height")){
  #df<-read.table(paste(outDIR, sprintf("%s/joint/torus_joint_enrichment.est", i),sep="/"), header = T)
  assign(i, plot_enrichment_est(i))
}
ggarrange(bmi, height, labels=c("bmi", "height"))
for(i in c("ibd_IBD","ibd_UC", "ibd_CD")){
  #df<-read.table(paste(outDIR, sprintf("%s/joint/torus_joint_enrichment.est", i),sep="/"), header = T)
  assign(i, plot_enrichment_est(i))
}
ggarrange(ibd_UC, ibd_CD, labels=c("ibd_UC", "ibd_CD"), ncol=2)

for(i in c("LDL","HDL")){
  #df<-read.table(paste(outDIR, sprintf("%s/joint/torus_joint_enrichment.est", i),sep="/"), header = T)
  assign(i, plot_enrichment_est(i))
}
ggarrange(LDL, HDL, labels=c("LDL", "HDL"))
assign("scz_2018", plot_enrichment_est("scz_2018"))
print("Schizophrenia")
scz_2018
```
