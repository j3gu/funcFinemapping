---
title: 'Lab 4: Fine-mapping with SuSiE'
output:
  pdf_document: default
  html_document:
    df_print: paged
---
*Created by Jing Gu*

SuSiE is a state-of-art fine-mapping method based on Bayesian variable selection regression. The name of "SuSiE" means the sum of single effects, which detects single causal effect one at a time but also extends to multiple effects through conditional analysis. 

SusieR package was developed to run fine-mapping procedures over GWAS summary statistics or invidiual level genotype/phenotype data. More details about the SusieR package can be found at [SusieR github page] [https://stephenslab.github.io/susieR/index.html].

By the end of the lab, you should be able to:

- **Understand the types of input files for running susieR**
- **Interpret output results from susieR**
- **Evaluate the effects of different number of causal variants on the results**
- **Compare the effects of different LD reference panels on the results**

# Setup

## download packages
**First, let's download necessary packages for today's lab session**
```{r echo=FALSE}
pkgs<-c("susieR", "data.table","dplyr","bigsnpr", "ggplot2")
for(i in pkgs){
  if(!require(i)){
    install.packages(i)
  }
}
library(susieR)
library(data.table)
library(dplyr)
library(bigsnpr)
library(ggplot2)
data("~/projects/funcFinemapping/transfer/susie.RData")
attach("~/projects/funcFinemapping/transfer/susie.RData")
```

## Dataset
* gwas summary statistics (after QC):
    - Body mass index as the phenotype
    - A meta-analysis of Locke et al 2015 and analyses from UK Biobank
    - No individual genotypes are provided.
    - Chr, pos, rsID, effect/non-effect alleles, beta, se(beta), p-values, odds ratio, N, etc. 

* reference LD panels from 1000 genome or UK Biobank
    - chr, pos, rsID, major/minor alleles, etc.
    - LD matrix: pair-wise LD coefficients across each LD block
  
* LD blocks:
    - Based on the European haplotypes, researchers have built 1704 LD blocks, where variants across blocks are not correlated. 
  
  
## Input files for susieR

* individual level phenotypes/genotypes: `susie()` or Summary statistics: `susie_rss()`
* LD coefficients (R)
* expected number of causal variants (L)
  

# Run susieR on the simulated example 
Let's first test the performance of susieR on the simulated dataset. There are two simulation replicates in the dataset. We will focus on the first one. 

**load N3finemapping from susieR package**
```{r}
data(N3finemapping)
attach(N3finemapping)
str(N3finemapping)
```
We can identify which variables are simulated as truly causal by searching for $b\neq0$.
```{r}
which(true_coef[,1] !=0)
plot(true_coef[,2], ylab = "true effect size")
```
**Run linear regression to obtain p-values, similar as running GWAS**
```{r}
sumstats <- univariate_regression(X, Y[,1])
z_scores <- sumstats$betahat / sumstats$sebetahat
susie_plot(z_scores, y = "z", b = true_coef[,1])
```
**Fine-mapping with susieR at L = 1**

The susie_plot() function is a scatter plot of PIP or z scores against variant index. The variables selected into credible sets are colored by groups. 
```{r}
fitted.L1 <- susie(X, Y[,1],
                L = 1,
        verbose = TRUE)
susie_plot(fitted.L1, y="PIP")
```
**Fine-mapping with susieR at L = 10**
```{r}
fitted.L10 <- susie(X, Y[,1],
                L = 10,
        verbose = TRUE)
susie_plot(fitted.L10, y="PIP")
```
By default, a credible set is defined as the minimum set of variables of which PIPs sum up to be at least 95%, and the minimum mean correlations within a set to be 0.5.

Problem 1: 
With seting L = 10, are the causal variables included in the credible sets?
```{r}
print(fitted.L10$sets)
```
It has been shown through simulations that the finemapping results are robust to the choices of L. 

# Fine-mapping on GWAS summary statistics

Let's first take a look at GWAS summary statistics file. There is one additional column that specifies the LD block that each variant belongs.
```{r}
head(gwas)
gwas<-split(gwas, gwas$locus)
```
By plotting -log10(p) against genomic positions, we see many SNPs near the lead SNP passing the significance threashold. To help prioritize SNPs for downstream analysis and validation, fine-mapping provides a statistical approach to evaluate each SNP's causal probability.
```{r}
orig.gwas<-gwas[["1452"]]
plot(x = orig.gwas$pos, y = -log10(orig.gwas$pval), 
     xlab = "pos", ylab="-log10(p)")
```
## Step 1: match variants between GWAS summary stats and LD panel
Bigsnpr package is very useful to process genotype data. We will use its "snp_match" function to match variants in GWAS summary stats and LD panels. This function outputs a summary table with previous GWAS summary statistics for the matched SNPs and additional columns such as the "bigSNP_index", the index number for each matched variant from the LD panels. We will further utilize this index numbers to quickly extract the LD coefficients from the LD matrix. 
```{r}
matched.gwas<-as_tibble(bigsnpr::snp_match(sumstats = gwas[["1452"]], 
                          info_snp = bigSNP[["1452"]]$map) %>% 
                          dplyr::rename(og_index = `_NUM_ID_.ss`) %>% 
                          dplyr::rename(bigSNP_index = `_NUM_ID_`))
# commons<-intersect(orig.gwas$snp, matched.gwas$snp)
# plot(x = orig.gwas[orig.gwas$snp %in% commons,]$beta,
#      y = matched.gwas[matched.gwas$snp %in% commons,]$beta,
#      xlab = "original beta", ylab = "SNP-matched beta")
```
Problem 2:
From the help page of "snp_match" function, we know when the alleles are reversed the direction of beta values will be reversed as well. Let's make a scatter plot comparing the beta values from original gwas and from the matched gwas. 

Problem 3: 
Make the same plot for z-scores. The directions of z scores can have a big impact on susie's performance. So be careful about specifying the right z-scores used in susie. Otherwise, you can directly compute zscores by dividing beta from se(beta). This relationship also implies the direction of z scores should be the same as beta. 

Knowing the directions of z scores were not updated, we manually computed the z scores in the following codes and repeated this step by looping over all the LD blocks. 
```{r}
for(z in names(gwas)){
  gwas[[z]]<-as_tibble(bigsnpr::snp_match(sumstats = gwas[[z]], 
                          info_snp = bigSNP[[z]]$map) %>% 
                          dplyr::rename(og_index = `_NUM_ID_.ss`) %>% 
                          dplyr::rename(bigSNP_index = `_NUM_ID_`)) %>%
                          dplyr::mutate(z_matched = beta/se)
}
```
## step2: Extract LD coefficients for the tested SNPs

The given LD matrix for each LD block is a N by N matrix, where N is the number of common variants within the block. The row and column index match with the "bigSNP_index" in the summary table output from snp_match function.
```{r}
snps.vec<-gwas[["1452"]]$bigSNP_index
r2.mat<-bigSNP[["1452"]]$LD_matrix[snps.vec, snps.vec]
#check if the bigSNP_index matches between GWAS and the LD panel
sum(gwas[["1452"]]$snp != bigSNP[["1452"]]$map[snps.vec,"id"])
```
## step 3: run susie_rss() for summary statistics

Let's first try L=1. Here we sepcified z scores as the column of "z_matched", which was corrected by directions.
```{r}
res.L1 <- susieR::susie_rss(z = gwas[["1452"]]$z_matched, R = r2.mat, L = 1, verbose = F)
summary(res.L1)
res.L1[["z"]]<-gwas[["1452"]]$z_matched
```
From the output summary, we know one credible set was identified. There are two variants, index 482 and 492, included in the credible set, each with around 0.5 posterior probability. As their average LD correlation is very close to one, Susie was not able to distinguish between them. Therefore, the probability of being causal given data were equally distributed between these two variables. 
Problem 

Problem 4:
Run susie at L=10 and then report the number of variables selected in each credible set. Additionally, use susie_plot() to visualize the distributions of PIPs and -log10(pval).
```{r}
res.L10 <- susieR::susie_rss(z = gwas[["1452"]]$z_matched, R = r2.mat, L = 10, verbose = F, track_fit = TRUE)
summary(res.L10)
res.L10[["z"]]<-gwas[["1452"]]$z_matched
par(mfrow = c(1,2))
susie_plot(res.L10, "PIP", add_legend = "topleft")
susie_plot(res.L10, y="z")
```
Now we are done with the general procedures for running susie. Let's put them together and loop over all the blocks.
```{r}
susie.ukb.L10<-list()
for(i in names(gwas)){
  snps.vec<-gwas[[i]]$bigSNP_index
  r2.mat<-bigSNP[[i]]$LD_matrix[snps.vec, snps.vec]
  susie.ukb.L10[[i]]<-susie_rss(z = gwas[[i]]$z_matched, R = r2.mat, L = 10, verbose = F)
  susie.ukb.L10[[i]][["z"]]<-gwas[[i]]$z_matched #add the original z scores to susie outputs
}
```
# Compare different LD panels
Lastly, we will examine how the mismatch of LD matrix can potentially lead to spurious results. Our previous analysis was based on the LD matrix computed over more than 40k British individuals provided by UK Biobank. In the dataset, we also provided the fine-mapped results using the LD matrix from samples in 1000 genome consortium. The LD matrix was computed over 500 Eurpean individuals. 


Let's focus on one specific LD block 1488 and see how the results differ due to the use of different LD reference panels. 

Problem 5.
Compare the credible sets run over ukbb LD matrix and 1kg LD matrix. The susie outputs run using 1kg LD matrix are stored in a variable called `susie.1kg.L10`. Also make the plots of PIPs or -log10(p-val) to show the difference. 
**1000 genome reference panels**
```{r}
summary(susie.1kg.L10$`1488`$susie.out)
par(mfrow = c(1,2))
susie_plot(susie.1kg.L10$`1488`$susie.out, y="PIP")
susie_plot(susie.1kg.L10$`1488`$susie.out, y="z")
```
**ukb reference panels:**
```{r}
summary(susie.ukb.L10$`1488`$susie.out)
par(mfrow = c(1,2))
susie_plot(susie.ukb.L10$`1488`, y="PIP")
susie_plot(susie.ukb.L10$`1488`, y="z")
```
**Check the LD patterns relative to the top GWAS SNP**
We can also visualize LD patterns by coloring the data points by the LD coefficients relative to the SNP of interest. 
```{r}
summary(susie.1kg.L10[["1488"]]$susie.out)
ggplot(susie.1kg.L10[["1488"]]$sumstats, aes(x=pos,y=-log10(pval), color=susie.1kg.L10$`1488`$LD.coef[383,])) +
       geom_point() + scale_color_gradient(low = alpha("yellow", 0.2), high ="red") + 
       labs(color="LD coefficient") + labs(title = "1kg") + 
       ylab("-log10(p-val)") +  xlab("variable") 

summary(susie.ukb.L10[["1488"]])
ggplot(gwas[["1488"]], aes(x=pos,y=-log10(pval), color=bigSNP$`1488`$LD_matrix[gwas[["1488"]][383,]$bigSNP_index, gwas[["1488"]]$bigSNP_index])) +
       geom_point() + scale_color_gradient(low = alpha("yellow", 0.2), high ="red") + 
       labs(color="LD coefficient") + labs(title = "UKB") + 
       ylab("-log10(p-val)") +  xlab("variable") 
```


